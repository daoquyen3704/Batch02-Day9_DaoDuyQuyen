...
Graph: analyze_law -> check_routing -> parallel [call_privacy, call_tax, call_compliance] -> aggregate -> END
"""
    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true -> question involves data privacy, GDPR, CCPA, consent, user data'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    privacy_prompt = (
        "You are a privacy and data protection specialist with expertise in GDPR, CCPA, "
        "data processing consent, breach notification, and privacy compliance. "
...
def create_graph():
    ...
    graph.add_conditional_edges(
        ...
        ["call_privacy_specialist", "call_tax_specialist", "call_compliance_specialist", "aggregate"],
    )
...
def save_graph_visualization(graph) -> None:
    """Render the graph to a PNG file so it can be inspected outside notebooks."""
    output_path = Path(__file__).with_name("stage_4_graph.png")
    try:
        rendered = graph.get_graph().draw_mermaid_png()
        ...
        print(f"Could not render graph visualization: {exc}")
...
async def main():
    ...
    print("  3. Privacy + Tax + Compliance specialists run IN PARALLEL (LangGraph Send API)")
    print("  4. Aggregator combines all analyses into a final answer")
    print()
    print("[Graph topology]")
    print("  analyze_law -> check_routing -> [call_privacy + call_tax + call_compliance] -> aggregate -> END")
    print()
